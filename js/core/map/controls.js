define(['OpenLayers','mapStyles','features','marker','georeference','popup','poi','modal','config','dataSource','request','notification','cluster'],function(OL,mapStyle,Features,Marker,Georeference,Popup,Poi,Modal,config,dataSource,Request,Notification,Cluster) {
	...
	var g = Map.getLayer('Georeference');
        ...
        streetv: new (getClickEvent(function(e){
			//elimina_vec();
			//alert('aqui');
		    Marker.event({action:'delete',items:'all',type:'identify'});
		    var lonlat = Map.map.getLonLatFromViewPortPx(e.xy);
		    var params = {lon:lonlat.lon,lat:lonlat.lat,type:'identify',params:{nom:'Street View',desc:'descripcion'}};
		    Marker.add(params);
			var nlocation=new OpenLayers.LonLat(lonlat.lon,lonlat.lat).transform(new OpenLayers.Projection("EPSG:900913"),new OpenLayers.Projection("EPSG:4326"));
            cargaImagen(nlocation.lat,nlocation.lon);
		    },Map
		    )),
        stobra: new (getClickEvent(function(e){
		    Marker.event({action:'delete',items:'all',type:'identify'});
		    var lonlat = Map.map.getLonLatFromViewPortPx(e.xy);
		    var params = {lon:lonlat.lon,lat:lonlat.lat,type:'identify',params:{nom:'Street View',desc:'descripcion'}};
		    Marker.add(params);
			var nlocation=new OpenLayers.LonLat(lonlat.lon,lonlat.lat).transform(new OpenLayers.Projection("EPSG:900913"));
            capturaObra(nlocation.lat,nlocation.lon);
		    },Map
		    )),
