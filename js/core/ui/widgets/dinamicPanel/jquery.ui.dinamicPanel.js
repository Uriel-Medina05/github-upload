...
$.widget("custom.dinamicPanel", {
     ...
     panels:[{id:'searchPanel',type:'search',height:180,label:'Resultado',desc:'Resultados de la búsqueda',icon:'dinamicPanel-sprite dinamicPanel-search-big'},
             {id:'measurePanel',type:'measure',height:180,label:'Medir',icon:'dinamicPanel-sprite dinamicPanel-measure-big',desc:'Realiza mediciones en el mapa', data:{},list:[]},
             {id:'bufferPanel',type:'buffer',height:165,label:'Análisis',desc:'Análisis espacial',icon:'dinamicPanel-sprite dinamicPanel-buffer-big'},
             {id:'leyendPanel',type:'leyend',height:460,label:'Leyenda',desc:'Significado de la ssimbología del mapa',icon:'dinamicPanel-sprite dinamicPanel-leyend-big'},
             {id:'geoPanel',type:'geo',height:200,label:'Georref.',desc:'Digitalizar en el mapa', icon:'dinamicPanel-sprite dinamicPanel-geo-big'},			 
             {id:'routePanel',type:'route',height:105,label:'Ruteo',desc:'Crear rutas por las vías principales del pa&iacute;s',icon:'dinamicPanel-sprite dinamicPanel-route-big'},			 
			 {id:'herramienta',type:'etools',height:180,label:'Obra Pública',desc:'Herramientas', icon:'dinamicPanel-sprite dinamicPanel-herramientas-big'}
             ],
     lastSearch:'',
     id:'',
     searchTimer:0,
     ...
     panelTransparency:function(opc){
        ...
        if (opc === undefined){
            $('#'+obj.id+'_toolSet').css('opacity',0.9);
			$('#'+obj.id+'_minitools').css('opacity',0.9);
     ...
     createPanel:function(type,data){
        ...
        switch (type) {
			 case 'eagrupa':	
				umap=obj.options.map;
				instrucciona = "window.open('../agp/','Agrupamientos')";
				cadena='';
				cadena+= '<center><table>';
				cadena+= '<tr>'								
				cadena+= '<td align="center"><button style="cursor:pointer" class="textbuttonagrup" title="Acceso a Herramienta Agrupamientos" onclick="'+instrucciona+'">Agrupamientos</button></td>'
				cadena+= '</tr>'
				cadena+= '</table>'								
				cadena+= '<h3 style="background: #316FBC; color:white">CLUSTERS/GRUPOS</h3>';
				cadena+= '<iframe id="datos" name="ndatos" frameBorder="0" height="385" width="370" src="php/grupos.php"></iframe>';
				//cadena+= '<button id="tema" style="font-size: 11px;" class="button ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only"><span class="ui-button-text">Ver grupo</span></button>';
				//cadena+= '<button id="eltema" style="font-size: 11px;" class="button ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only"><span class="ui-button-text">Borrar grupo</span></button>';
				$('#'+obj.id+'_agrupa').html(cadena);				
				//$('#tema').click(function(){   
						//alert('pintar grupo');
			//			obj.options.map.cargaGrupo();
			//	});
			//	$('#eltema').click(function(){ 
						//alert('Eliminar grupo');
		//				obj.options.map.borraGrupo();
		//		});
			...
			case 'etools':	
				...
				var obj = this;				
				cadena='';
				cadena+= '<center><h3 style="background: #316FBC; color:white">Herramientas</h3>';                   
				cadena+= '<table border="1">';
				cadena+= '<tr>'
				cadena+= '<td width="25%" align="center"><span id="'+obj.id+'_point_btnstreet" style="cursor:pointer" title="Imágenes de calle" class="dinamicPanel-tabIcon dinamicPanel-sprite dinamicPanel-street-big"></span></td>'
                cadena+= '<td width="25%" align="center"><span id="'+obj.id+'_point_btnobra"   style="cursor:pointer" title="Obra Pública" class="dinamicPanel-tabIcon dinamicPanel-sprite dinamicPanel-street-big"></span></td>'
                cadena+= '<td width="25%" align="center"><span id="'+obj.id+'_point_btnobra"   style="cursor:pointer" title="Obra Pública" class="dinamicPanel-tabIcon dinamicPanel-sprite dinamicPanel-street-big"></span></td>'
				cadena+= '</tr>'
				cadena+= '<tr><td align="center" id="tdstreet"><span class="dinamicPanel-tabDescription">Imágenes de calle</span></td>'			
				cadena+= '<td align="center" id="tdobra"><span class="dinamicPanel-tabDescription">Registra obra</span></td>'
                cadena+= '<td align="center" id="tdobra"><span class="dinamicPanel-tabDescription">Registra obra</span></td></tr>'
				cadena+= '</table>';						  				
				$('#'+obj.id+'_herramienta').html(cadena);
				
                document.getElementById('tdstreet').style.backgroundColor = "";
				
                $("#"+obj.id+'_point_btnstreet').click(function(){   
                    document.getElementById('tdstreet').style.backgroundColor = "#C4D1DD";   
                    document.getElementById('tdobra').style.backgroundColor = "";          										
					var map = obj.options.map;
					map.activeCtl({control:"streetv",active:true});					 
                }); 

                $("#"+obj.id+'_point_btnobra').click(function(){  
                    document.getElementById('tdstreet').style.backgroundColor = "";   
                    document.getElementById('tdobra').style.backgroundColor = "#C4D1DD";     
                    var map = obj.options.map;
                    map.activeCtl({control:"stobra",active:true});                  
                });
    ...
    getAbstract:function(data){
     ...
     createTabs:function(){
        var obj = this;
        var cadena = '';
        //var width = 16;//Math.floor(100/obj.panels.length);
        var width = 12;//Math.floor(100/obj.panels.length);
        for (var x in obj.panels){
